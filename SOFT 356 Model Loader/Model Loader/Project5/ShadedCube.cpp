...
void
init(vector<GLfloat>& vertices, vector<GLfloat>& textures, vector<GLfloat>& normals, vector<GLfloat>& colour, string texture_name, vector<GLfloat>& diffuse, vector<GLfloat>& specular, GLfloat& ns)
{
	...
	/*vector < glm::vec4> ambientColour;
	for (size_t i = 0; i < vertices.size() / 3; i++)
	{
		ambientColour.push_back(ambient);
	}*/
	...
	glBufferStorage(GL_ARRAY_BUFFER, sizeof(colours), colours, GL_STATIC_DRAW);
...
//	glBindBuffer(GL_ARRAY_BUFFER, Buffers[Normals]);
	//glBufferStorage(GL_ARRAY_BUFFER, normals.size() * sizeof(GLfloat), normals.data(), GL_STATIC_DRAW);
...
main(int argc, char** argv)
{
	...
	init(vertices, textures, normals, colour, texture_name, diffuse, specular, ns);
	GLfloat timer = 0.0f;
